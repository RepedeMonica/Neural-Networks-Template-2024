...
def initialization():
    ...
    limit_W1 = np.sqrt(6/ (784 + 100))
    W1 = np.random.uniform(-limit_W1, limit_W1, (784, 100))
    b1 = np.zeros((1, 100))

    limit_W2 = np.sqrt(6 / (100 + 10))
    W2 = np.random.uniform(-limit_W2, limit_W2, (100, 10))
    b2 = np.zeros((1, 10))
...
def forward(X, W1, b1, W2, b2, dropout_rate=0.2, training = True): #80% raman activi
...
def backward(X, Y, A1, A2, W1, W2 , b1, b2, learning_rate=0.01):
    m = X.shape[0]

    #slide 36,curs5 -dem pt formula
    dZ2 = A2 - Y  #eroarea pt final layer, formula slide 44, curs5 #derivata softmax
    dW2 = np.dot(A1.T, dZ2)  / m   #formula slide 46 - formula basic de calculare a w
...
def train(X, Y, W1, b1, W2, b2, epochs=200, batch_size=64, learning_rate=0.05):
    patience = 3
    decay_rate = 0.8
    best_accuracy = 0
    ...
    while epoch < epochs:
        ...
        for i in range(0, m, batch_size):
            ...
            Z1, A1, Z2, A2 = forward(X_batch, W1, b1, W2, b2, training=True)
            W1, b1, W2, b2 = backward(X_batch, y_batch, A1, A2, W1, W2, b1, b2, learning_rate)

        _, _, _, A2_train = forward(X, W1, b1, W2, b2)
        _, _, _, A2_test = forward(test_X, W1, b1, W2, b2, training=False)

        train_acc = accuracy(Y, A2_train)
        test_acc = accuracy(test_Y_encoding, A2_test)

        elapsed_time = time.time() - start_time
        print(f"Epoch {epoch+1}/{epochs} - Train Accuracy: {train_acc*100:.2f}% - Test Accuracy: {test_acc*100:.2f}%  - Elapsed Time: {elapsed_time:.2f}")
...
W1_trained, b1_trained, W2_trained, b2_trained = train(train_X, train_Y_encoding, W1, b1, W2, b2)
